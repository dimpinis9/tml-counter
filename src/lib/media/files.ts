...
export const VIDEO_MAX_LABEL =
  VIDEO_MAX_MB % 1024 === 0
    ? `${VIDEO_MAX_MB / 1024} GB`
    : `${VIDEO_MAX_MB} MB`;
    ...
    return {
      ...
      error:
        mediaType === "photo"
          ...
          : `Videos must be ${VIDEO_MAX_LABEL} or smaller.`,
    };
  }
...
function getConfiguredVideoMaxMb() {
  ...
  return Number.isInteger(configured) && configured >= 1 && configured <= 2048
    ? configured
    : 50;
}
